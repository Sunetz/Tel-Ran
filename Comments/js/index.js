...
const messages = [
    ...
    new Message(2, 'uossss', 'SJDz.jnv>SLKV dm .lz..', new Date().getFullYear()),
    new Message(2, 'super kzdl/zm', 'super sDJvlnn ckmhoHO>NS>LhieohG>LVdKA>iuf;OHb,khvjo  auUFEUH', new Date().getFullYear()),
]
...
leftBox.onclick=(event)=>{
    ...
    console.log(`event.target.parentNode.dataset.message: ${event.target.parentNode.dataset.message}`); */
    
    const userName=users.find(item => +event.target.dataset.id === item.id);
    console.log(event.target.dataset.id)
    const usersMessage = messages.filter(item => +event.target.parentNode.dataset.message === item.id);
    if(event.target.tagName==='H4'){
        rightBox.innerHTML=usersMessage.map(item=>item.renderFullInfo(userName)).join('')
    }
} 