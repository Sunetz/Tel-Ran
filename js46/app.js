...
userList.forEach(u => u.onclick = userClickHandler);
...
function getAlbumsByUserId(userId){
  const element = document.createElement('div');
  const userAlbums = albums.filter(el => el.userId === userId);//collection
...
function albumClickHandler(e){
  ...
  right.append(getPhotoByAlbumId(albumId))
}
